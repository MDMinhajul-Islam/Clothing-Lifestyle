...
export const CustomerAuthModal:React.FC<Props>=({isOpen,onClose,currentCustomer,onUpdateCustomer,resetToken,initialNotice})=>{
  ...
  if(currentCustomer.verified){
    ...
    return <div className="fixed inset-0 z-50 grid place-items-center bg-black/55 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Customer account">
      <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-[#f6f6f4] shadow-[0_30px_100px_rgba(0,0,0,0.35)] md:flex-row">
        <aside className="bg-[#151719] p-5 text-white md:w-64 md:p-6">
          <div className="flex items-center justify-between"><div><p className="font-display text-lg font-bold tracking-[0.24em]">NEXGEN</p><p className="mt-1 text-[8px] uppercase tracking-[0.18em] text-neutral-500">Customer account</p></div><button onClick={onClose} className="rounded-full p-2 hover:bg-white/10 md:hidden" aria-label="Close"><X className="h-4 w-4"/></button></div>
          ...
          <button onClick={()=>void logout()} className="mt-5 flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs text-neutral-400 hover:bg-white/5 hover:text-white md:mt-10 md:w-full"><LogOut className="h-4 w-4"/>Logout</button>
        </aside>
        <section className="min-w-0 flex-1 overflow-y-auto">
          <header className="sticky top-0 z-10 flex items-center justify-between border-b border-neutral-200 bg-white/95 px-5 py-5 backdrop-blur sm:px-8"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-neutral-400">NexGen account</p><h2 className="mt-1 text-2xl font-semibold">{portalNav.find(([id])=>id===portalSection)?.[1]}</h2></div><button onClick={onClose} className="hidden rounded-full border border-neutral-200 p-2.5 hover:bg-neutral-50 md:block" aria-label="Close"><X className="h-4 w-4"/></button></header>
