...
export const CustomerAuthModal: React.FC<Props> = ({ isOpen, onClose, currentCustomer, onUpdateCustomer }) => {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Customer account">
    <div className="w-full max-w-md bg-white shadow-2xl">
      ...
      <div className="p-6">
        {currentCustomer.authLevel !== 'ANONYMOUS' && <div className="mb-5 space-y-4"><div className="border border-emerald-200 bg-emerald-50 p-4 text-sm"><strong>{currentCustomer.name}</strong><p className="mt-1 text-xs text-neutral-600">Signed in as {currentCustomer.email}</p></div><div className="grid grid-cols-2 gap-2">{['Profile', 'Order history', 'Saved addresses', 'Recently viewed'].map((item) => <button key={item} className="border border-neutral-200 p-3 text-left text-[10px] font-semibold uppercase tracking-wider hover:border-black">{item}</button>)}</div><p className="text-[10px] text-neutral-500">Wishlist and saved sizes will appear here when available.</p></div>}
