...
export const ProductGrid: React.FC<ProductGridProps> = ({ products, isLoading, error, onSelectProduct, onAskAboutProduct, onResetFilters, onAskVoice, voiceActionNotice, totalCatalogueCount = 6018, onLoadMore, hasMore = false, isLoadingMore = false }) => {
  ...
  return (
    <section className="mx-auto max-w-[1600px] px-4 py-9 sm:px-8 lg:px-12 2xl:px-16">
      ...
      <div className="mb-6 flex items-end justify-between"><div><span className="text-[10px] uppercase tracking-[0.2em] text-neutral-400">Browse the collection</span><h2 className="mt-1 font-serif-luxury text-2xl text-neutral-950">Curated now</h2></div><p className="text-right text-[10px] uppercase tracking-wider text-neutral-400">{products.length} shown<br /> {totalCatalogueCount.toLocaleString()}</p></div>
