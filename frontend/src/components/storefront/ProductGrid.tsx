...
interface ProductGridProps {
  ...
  totalCatalogueCount?: number; onLoadMore?: () => void; hasMore?: boolean; isLoadingMore?: boolean;
}

export const ProductGrid: React.FC<ProductGridProps> = ({ products, isLoading, error, onSelectProduct, onAskAboutProduct, onResetFilters, onAskVoice, voiceActionNotice, totalCatalogueCount = 6018, onLoadMore, hasMore = false, isLoadingMore = false }) => {
  ...
  return (
    <section className="mx-auto max-w-[1600px] px-4 py-9 sm:px-8 lg:px-12 2xl:px-16">
      ...
      <div className="mb-6 flex items-end justify-between"><div><span className="text-[10px] uppercase tracking-[0.2em] text-neutral-400">Browse the collection</span><h2 className="mt-1 font-serif-luxury text-2xl text-neutral-950">Curated now</h2></div><p className="text-right text-[10px] uppercase tracking-wider text-neutral-400">Previewing {products.length} pieces<br />Connected catalogue: {totalCatalogueCount.toLocaleString()}</p></div>
      ...
      <div className="mt-14 flex flex-col items-center gap-3 border-t border-neutral-200 pt-8">{hasMore ? <button onClick={onLoadMore} disabled={isLoadingMore} className="flex items-center gap-2 bg-black px-8 py-3.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-white disabled:opacity-50">{isLoadingMore ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}{isLoadingMore ? 'Loading pieces' : 'Load more styles'}</button> : <p className="text-[10px] uppercase tracking-widest text-neutral-400">End of this curated preview · ask by voice to discover another direction</p>}</div>
    </section>
  );
};
