  ...
  return (
    <article className="group flex flex-col bg-white">
      <button onClick={() => onSelect(product)} className="relative aspect-[3/4] overflow-hidden bg-neutral-100 text-left" aria-label={`View ${product.name}`}>
        ...
        <img src={product.image} alt={product.name} loading="lazy" onLoad={() => setLoaded(true)} className={`h-full w-full object-cover object-top transition duration-700 group-hover:scale-105 ${loaded ? 'opacity-100' : 'opacity-0'}`} />
      ...
      <div className="flex flex-1 flex-col py-3">
        <div className="mb-2 flex items-center gap-1.5">{product.colors.slice(0, 4).map((color) => <span key={color.name} className="h-2.5 w-2.5 rounded-full border border-neutral-300" style={{ backgroundColor: color.hex }} title={color.name} />)}<span className="ml-1 text-[9px] uppercase tracking-wider text-neutral-400">{product.colors[0]?.name}</span></div>
        ...
        <div className="mt-2 flex items-center justify-between border-t border-neutral-100 pt-2 text-[9px] uppercase tracking-wider text-neutral-400"><span>{product.sizes.slice(0, 4).join(' · ')}</span><span className={product.inStock ? 'text-emerald-700' : 'text-neutral-400'}>{product.inStock ? 'Available' : 'Check availability'}</span></div>
      </div>
    </article>
  );
};
