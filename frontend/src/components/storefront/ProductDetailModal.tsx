...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ product, isLoading = false, error, onClose, onAddToCart, onAskVoicePrompt, matchingProducts, onSelectMatchingProduct }) => {
  ...
  const selectedSize = sizeSelection?.productId === product.id ? sizeSelection.value : product.sizes[0] || 'One size';
      ...
      <article className="relative z-10 min-h-screen w-full max-w-3xl overflow-y-auto border-l border-neutral-200 bg-white shadow-2xl">
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.15fr_0.85fr] md:p-8">
          ...
          <div className="space-y-7">
            <div><p className="text-[10px] uppercase tracking-[0.2em] text-neutral-400">{product.department || 'Information unavailable'} / {product.category || 'Information unavailable'}</p><h2 className="mt-2 font-serif-luxury text-3xl leading-tight text-neutral-950">{product.name || 'Information unavailable'}</h2><p className="mt-3 font-mono text-lg">${product.price.toFixed(2)} <span className="text-xs text-neutral-400">{product.currency}</span></p><p className="mt-5 text-sm leading-relaxed text-neutral-600">{product.longDescription || product.description || 'Information unavailable'}</p>{error && <p className="mt-3 text-xs text-amber-800">{error}</p>}</div>
            ...
            <div><div className="mb-3 flex justify-between text-[10px] uppercase tracking-[0.15em]"><span>Available sizes</span><span>{product.sizes.length ? selectedSize : 'Information unavailable'}</span></div>{product.sizes.length ? <div className="grid grid-cols-4 gap-2">{product.sizes.map((size) => <button key={size} onClick={() => setSizeSelection({ productId: product.id, value: size })} className={`border py-2.5 text-xs ${selectedSize === size ? 'border-black bg-black text-white' : 'border-neutral-200 hover:border-black'}`}>{size}</button>)}</div> : <p className="text-xs text-neutral-500">Information unavailable</p>}</div>
            <div><h3 className="mb-3 text-[10px] uppercase tracking-[0.15em]">Available colors</h3>{product.colors.length ? <div className="flex flex-wrap gap-2">{product.colors.map((color) => <span key={color.name} className="inline-flex items-center gap-2 border border-neutral-200 px-3 py-2 text-xs"><span className="h-3 w-3 rounded-full border border-black/20" style={{ backgroundColor: color.hex }} />{color.name}</span>)}</div> : <p className="text-xs text-neutral-500">Information unavailable</p>}</div>
