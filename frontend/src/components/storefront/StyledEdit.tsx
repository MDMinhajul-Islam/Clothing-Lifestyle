...
export const StyledEdit = ({ products, onSelectProduct, onAskVoice }: StyledEditProps) => {
  ...
  return (
    <section className="overflow-hidden bg-[#171714] py-16 text-white lg:py-24">
      <div className="mx-auto max-w-[1700px] px-4 sm:px-8 lg:px-12 2xl:px-16">
        <div className="mb-10 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-emerald-400"><Sparkles className="h-3.5 w-3.5" />AI-curated looks</p><h2 className="mt-3 font-serif-luxury text-4xl font-light sm:text-6xl">The styled edit</h2></div><button onClick={() => onAskVoice('Style a complete look for me')} className="self-start border-b border-white/50 pb-1 text-[10px] uppercase tracking-[0.18em] hover:border-emerald-400 hover:text-emerald-400">Style this by voice</button></div>
