...
const FASHION_PRODUCTS = MOCK_PRODUCTS.filter(isMainFashionProduct);
const AVAILABLE_COLORS = Array.from(new Map(FASHION_PRODUCTS.flatMap((product) => product.colors).map((color) => [color.name, color])).values());
...
export const App: React.FC = () => {
  ...
  const filteredProducts = React.useMemo(() => FASHION_PRODUCTS.filter((product) => {
    ...
    if (filter.searchQuery && !matchesProductSearch(product, filter.searchQuery)) return false;
    return true;
  ...
  const sendTranscript = React.useCallback(async (transcript: string) => {
    ...
    if (!session) return;
    const result = await sendVoiceTurn(session, text, FASHION_PRODUCTS);
    ...
    if (result.openProductDetail) { setSelectedProduct(result.openProductDetail); setMatchingProducts(compatibleProducts(result.openProductDetail, FASHION_PRODUCTS)); }
    if (result.matchingProducts?.length) setMatchingProducts(result.matchingProducts.filter(isMainFashionProduct));
  ...
  const selectProduct = (product: Product) => { setSelectedProduct(product); setMatchingProducts(compatibleProducts(product, FASHION_PRODUCTS)); };
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#fcfcfc] text-neutral-900">
      ...
      <Hero featuredProducts={FASHION_PRODUCTS.filter((product) => product.department === 'WOMAN').slice(0, 3)} onStartVoice={() => void startVoice()} onExploreCollection={() => window.scrollTo({ top: 720, behavior: 'smooth' })} onSelectPrompt={(prompt) => void sendTranscript(prompt)} />
      <StyledEdit products={FASHION_PRODUCTS.filter((product) => product.department === 'WOMAN')} onSelectProduct={selectProduct} onAskVoice={(prompt) => void sendTranscript(prompt)} />
      <FilterBar filter={filter} onChangeFilter={(next) => { setFilter(next); setVisibleCount(PAGE_SIZE); }} onResetFilter={resetFilters} totalResults={filteredProducts.length} totalCatalogueCount={CONNECTED_CATALOGUE_COUNT} activeVoiceFilterLabel={activeVoiceFilterLabel} onClearVoiceFilter={resetFilters} availableColors={AVAILABLE_COLORS} />
