...
export const App: React.FC = () => {
  ...
  const selectProduct = async (product: Product) => { const requestId = ++detailRequest.current; setSelectedProduct(product); setIsProductDetailLoading(!usingFallback); setProductDetailError(null); const recommendationPool = Array.from(new Map([...products, ...styledProducts].map((item) => [item.id, item])).values()); try { const details = usingFallback ? product : await fetchProductDetails(product.id); if (requestId !== detailRequest.current) return; setSelectedProduct(details); setMatchingProducts(compatibleProducts(details, recommendationPool)); } catch { if (requestId !== detailRequest.current) return; setProductDetailError('Some product information is unavailable.'); setMatchingProducts(compatibleProducts(product, recommendationPool.length ? recommendationPool : FALLBACK_PRODUCTS)); } finally { if (requestId === detailRequest.current) setIsProductDetailLoading(false); } };
