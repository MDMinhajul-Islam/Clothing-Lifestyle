...
export const App: React.FC = () => {
  ...
  const webpageVoiceContext = React.useCallback((): WebpageVoiceContext => {
    const matched = selectedProduct?.matchedVariant;
    return {
      product_id: selectedProduct?.id,
      reference_product_id: selectedProduct?.id,
      active_variant_id: matched?.id,
      product_reference: selectedProduct?.commercialReference,
      sku: matched?.sku,
      color: matched?.color,
      size: matched?.size,
  ...
  const startVoice = React.useCallback(async () => {
    ...
    try {
      ...
      const authorization = await createRetellWebCall(customer.id || undefined, webpageVoiceContext());
  ...
  const sendTranscript = React.useCallback(async (transcript: string) => { const text = transcript.trim(); if (!text) return; const session = voiceSession?.status === 'ACTIVE' ? voiceSession : await startVoice(); if (!session) return; if (session.provider === 'retell') { window.speechSynthesis.cancel(); setAuthNotice('The live stylist is connected. Say your request naturally.'); return; } setTurnHistory((history) => [...history, { id: `user-${Date.now()}`, sender: 'user', text, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]); setVoiceState('THINKING'); setIsVoiceExpanded(true); const productIntent = voiceFilter(text); if (Object.keys(productIntent).length) { setFilter((current) => ({ ...current, ...productIntent })); setActiveVoiceFilterLabel(text); } const result = await sendVoiceTurn(session, text, products.length ? products : FALLBACK_PRODUCTS, webpageVoiceContext()); if (result.updatedFilter) { setFilter((current) => ({ ...current, ...result.updatedFilter })); setActiveVoiceFilterLabel(text); } setLastTurn(result.message); setTurnHistory((history) => [...history, result.message]); setVoiceState('SPEAKING'); window.speechSynthesis.cancel(); const speech = new SpeechSynthesisUtterance(result.message.text); speech.rate = 1.05; speech.onend = () => setVoiceState('LISTENING'); speech.onerror = () => setVoiceState('LISTENING'); window.speechSynthesis.speak(speech); }, [products, startVoice, voiceSession, webpageVoiceContext]);
  ...
  return <div className="flex min-h-screen flex-col bg-[#fcfcfc] text-neutral-900">
    ...
    <ProductDetailModal product={selectedProduct} isLoading={isProductDetailLoading} error={productDetailError} onClose={closeProduct} onAddToCart={(product, size) => setCartItems((items) => [...items, { product, size }])} onAskVoicePrompt={(prompt) => void sendTranscript(prompt)} matchingProducts={matchingProducts} onSelectMatchingProduct={(product) => void selectProduct(product)} onContextChange={setSelectedProduct} />
