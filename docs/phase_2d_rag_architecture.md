...
Status: Phase 2D.1 complete with local policy embeddings and hybrid retrieval validation. Existing backend policy rules are unchanged. No orchestrator, voice, frontend, recommendation embeddings or catalogue sync was implemented.
...
Migration 008 was executed twice and the corpus imported twice inside a successful rollback verification transaction. It was then applied with 11 documents and 11 chunks. Phase 2D.1 populated all 11 chunks with verified 384-dimensional vectors. Client privileges/RLS were checked. Counts for the checked existing catalogue, orders, returns, inventory and gateway tables stayed unchanged.

## Local embeddings

Phase 2D.1 uses `local_sentence_transformers`, model `sentence-transformers/all-MiniLM-L6-v2`, dimension 384, version `v1`. All 11 existing policy chunks were embedded locally on CPU; CUDA is selected automatically when PyTorch reports it available. No API key, paid service or remote embedding endpoint is used. Model files are loaded from the local cache after the initial free model download.

Vectors are normalized before storage. Dimension, finiteness and nonzero magnitude are validated. Cached vectors are reused only when chunk hash and model identity match. `requirements-rag.txt` pins the local embedding dependency.
...
Hybrid retrieval uses exact cosine candidates plus full-text candidates and reciprocal-rank fusion. A 0.30 MiniLM cosine candidate cutoff was the single ranking adjustment. Scores are ranking signals, not truth probabilities. Every meaningful normalized query term must be present in returned evidence; this conservative lexical grounding preserves abstention and may reject paraphrases. Semantic-only candidates are not admitted independently.
...
The nine grounded retrieval questions test expected source, topic, article relevance and citation completeness. Three negative questions test abstention. Live read-only hybrid retrieval achieved 9/9 expected sources at rank one and 3/3 abstentions. Hybrid matched lexical on eight questions and improved “Are there return exceptions?” from the general return article to the expected special-conditions article. This small corpus smoke test is not a general quality claim.
...
To regenerate idempotently, run `generate_embeddings`, `import_knowledge --apply`, and the read-only `evaluate_retrieval` command.
...
Scope ends here. Handle documented rule/security changes as explicit reviewed follow-ups before Phase 2E relies on them.
